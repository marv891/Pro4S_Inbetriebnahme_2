...
class GUI_functions(QWidget):
    ...
    def mousePressed(self, QMouseEvent):
        ...
        if keyboard.is_pressed('shift'):
            # self.laserImage = 'MicrosoftTeams-image.png'
            self.laserImage = CameraControl.getimageBGR()
            self.xLaser, self.yLaser = bver_algorithm_auto_lasercross.mainLaserCrossDetection(self.laserImage)
    ...
    def getMeasuringPhantomImage(self):
        ...
        # measuringArrayImage = 'MicrosoftTeams-image (1).png'
        nameImage = self.getActualTimeFilename()
        distance = bver_algorithm_auto_corner.main(1056, 392, measuringArrayImage, self.laserImage, self.xLaser, self.yLaser, nameImage)
